...
const ProductCard = ({ name, price, rating, image, isNew, link, category }) => (
    <article className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition duration-300 border border-gray-100 overflow-hidden group">
        ...
        <div className="p-5">
            <span className="text-xs text-primary-600 bg-primary-50 px-2 py-1 rounded-full font-medium">{category}</span>
            <h3 className="font-bold text-lg text-gray-800 mt-2 mb-1">
                <a href={link} target="_blank" rel="noopener noreferrer" className="hover:text-primary-600 transition">{name}</a>
            </h3>
...
const ProductsPage = () => {
    ...
    return (
        <>
            {/* Hero Banner */}
            <section className="bg-gradient-to-r from-primary-500 to-primary-700 py-16 text-white" aria-label="สินค้าทั้งหมด">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    ...
                    <nav className="flex flex-wrap justify-center gap-3" aria-label="หมวดหมู่สินค้า">
                        {categories.map((cat, i) => (
                            <button key={i} className={`px-5 py-2 rounded-full font-medium transition text-sm ${i === 0 ? 'bg-white text-primary-600' : 'bg-white/20 text-white hover:bg-white/30'}`}>
                                {cat}
                            </button>
                        ))}
                    </nav>
            ...
            {/* Products Grid */}
            <section className="py-16 bg-gray-50" aria-label="รายการสินค้า">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mt-16 text-center bg-white rounded-2xl p-10 shadow-sm border border-gray-100">
                        <h2 className="text-2xl font-bold text-gray-900 mb-3">ต้องการสินค้าเพิ่มเติม?</h2>
                        <p className="text-gray-600 mb-6">เรามีสินค้ามากกว่า 5,000+ รายการ สามารถสั่งซื้อได้ทุกช่องทาง</p>
