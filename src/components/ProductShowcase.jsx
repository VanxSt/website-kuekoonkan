...
const ProductCard = ({ name, price, rating, image, isNew, link }) => (
    <article className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition duration-300 border border-gray-100 overflow-hidden group flex flex-col h-full">
        <div className="relative h-40 sm:h-48 bg-gray-100 overflow-hidden shrink-0">
            {isNew && (
        ...
        <div className="p-4 sm:p-6 flex flex-col flex-grow">
            <div className="flex justify-between items-start mb-2 gap-2">
                <h3 className="font-bold text-sm sm:text-lg text-gray-800 line-clamp-2 sm:line-clamp-1">
            ...
            <p className="text-gray-500 text-xs sm:text-sm mb-4 flex-grow">หมวดหมู่: วัตถุดิบเบเกอรี่</p>

            <div className="flex justify-between items-center mt-auto">
                <span className="text-base sm:text-xl font-bold text-primary-600">฿{price}</span>
                <a href={link} target="_blank" rel="noopener noreferrer" className="p-1.5 sm:p-2 rounded-full bg-primary-50 text-primary-600 hover:bg-primary-500 hover:text-white transition" aria-label={`สั่งซื้อ ${name}`}>
